...
public class ShapesManager : MonoBehaviour
{
    ...
    public List<GameObject> shapesBGList;

    private int score;

    public  Vector2 BottomRight = new Vector2(-2.37f, -4.27f);
    public  Vector2 CandySize = new Vector2(0.7f, 0.7f);
    ...
    public void InitializeCandyAndSpawnPositionsFromPremadeLevel()
    {
        ...
        for (int row = 0; row < Constants.Rows; row++)
        {
            for (int column = 0; column < Constants.Columns; column++)
            {
                ...
                if (candyBackGround)
                {
                    InstantiateCandyBackground(row, column, candyBackGround);
                }
    ...
    private void InstantiateCandyBackground(int row, int column, GameObject candyBG)
    {
        GameObject go = Instantiate(candyBG,
    ...
    private void DestroyAllCandy()
    {
        ...
        if (candyBackGround)
        {
            foreach (var bg in shapesBGList)
            {
                Destroy(bg);
            }
        }
